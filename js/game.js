...
const gameClass = () => {
	...
	function consumeItem() {
		...
		/* eat consumable */
			...
			DATA.player.focus += parseInt(ITEMS[item_id][5] + Math.floor(Math.sqrt(stage_val) * 50 * (1+(thisItemObject.quality / 100)) * thisItemObject.grade * thisItemObject.grade)) || 0
			/* grimoire*/
	...
	const itemsInventoryRender = () => {
		for (var i = 0; i < DATA.player.inventory.length; i++) {
			...
			if (DATA.player.inventory[i].focus !== null) {
				let itemEffects = iText("item_food_focus", numberPrint(toDecimal(DATA.player.inventory[i].focus + Math.floor(Math.sqrt(DATA.player.inventory[i].stage) * 50 * (1+(DATA.player.inventory[i].quality / 100)) * DATA.player.inventory[i].grade * DATA.player.inventory[i].grade))))
